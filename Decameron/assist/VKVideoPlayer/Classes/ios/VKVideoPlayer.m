...
//    NSLog(@"timeInSeconds--%f  totalVideoDuration--%f",timeInSeconds,self.videoTrack.totalVideoDuration.floatValue);
    if (timeInSeconds > self.videoTrack.totalVideoDuration.floatValue - 0.1 && timeInSeconds > 3.0) {
        [self pauseContent];
    }
  ...
  if ([self.player currentItemDuration] > 1) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kVKVideoPlayerScrubberValueUpdatedNotification object:self.view userInfo:info];
    ...
    NSDictionary *durationInfo = [NSDictionary dictionaryWithObjectsAndKeys:
                                  ...
                                  nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:kVKVideoPlayerDurationDidLoadNotification object:self.view userInfo:durationInfo];
  }
...
- (void)loadVideoWithTrack:(id<VKVideoPlayerTrackProtocol>)track {
  ...
  switch (self.state) {
    ...
    case VKVideoPlayerStateContentPlaying:
          self.videoTrack.totalVideoDuration = [NSNumber numberWithFloat:5.0];
