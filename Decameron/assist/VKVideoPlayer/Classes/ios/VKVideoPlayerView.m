...
- (void)initialize {
  ...
  [defaultCenter addObserver:self selector:@selector(durationDidLoad:) name:kVKVideoPlayerDurationDidLoadNotification object:self];
...
- (void)durationDidLoad:(NSNotification *)notification {
  ...
  NSNumber* duration = [info objectForKey:@"duration"];
    ...
    duration = [NSNumber numberWithFloat:(durationFloat-4)];
